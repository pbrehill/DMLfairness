...
```{r}
make_facets <- function(data, variable, facet_var = NULL, dashed_line = "---------") {
  ...
  geom_boxplot() +
  stat_summary(aes(y = ability / mean(ability) * mean(data$value[data$estimate == 'Fair estimate'])), fun.y = mean, colour = "red", size = 5, geom = "text", label= dashed_line) +
  scale_y_continuous(
   name = "Predictions",
   sec.axis = sec_axis(~(./ mean(data$value[data$estimate == 'Fair estimate'])) * mean(data$ability), name="Average ability (latent)")
...
```{r}
# Race graph
make_facets(wakefield_long, race, dashed_line = "---")
